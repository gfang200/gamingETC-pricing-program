...
wb.Visible := True
	Loop, parse, A_LoopReadLine, `,
	{
		...
		{
		setName := A_LoopField
		;if tcgplayer url system ever changes go to this line
...
sleepTime =2000
		...
		Loop % check.Length
		{
			...
			{ 
				SoundPlay *-1
				wb.Visible := True
				MsgBox, Enter Captcha and hit OK to continue %count%
				wb.quit
				wb := ComObjCreate("InternetExplorer.Application")
				wb.Visible := True
				wb.Navigate(searchName)
...
;msgBox %uniqueID%
MsgBox,1,Gaming Etc Pricing Program,Step 2 has finished!
ExitApp
return
